...
static int cop1Emulate(struct pt_regs *regs, struct mips_fpu_soft_struct *ctx)
{
	...
	err = get_user(ir, (mips_instruction *) REG_TO_VA regs->cp0_epc);
	if (err) {
	...
	case cop1_op:
		...
		case bc_op: {
			...
			if (cond) {
				...
				err = get_user(ir,
					(mips_instruction *) REG_TO_VA regs->cp0_epc);
				if (err) {
					fpuemuprivate.stats.errors++;
					return SIGBUS;
				}
				 ...
				 * dslot
				 */
				return mips_dsemul(regs, ir, VA_TO_REG contpc);
			} else {
				/* branch not taken */
				if (likely) {
					/*
...
#define SIFROMREG(si,x)	((si) = \
			(xcp->cp0_status & ST0_FR) || !(x & 1) ? \
			ctx->regs[x] : \
			ctx->regs[x & ~1] >> 32 )
#define SITOREG(si,x)	(ctx->regs[x & ~((xcp->cp0_status & ST0_FR) == 0)] = \
			(xcp->cp0_status & ST0_FR) || !(x & 1) ? \
			ctx->regs[x & ~1] >> 32 << 32 | (u32)(si) : \
			ctx->regs[x & ~1] << 32 >> 32 | (u64)(si) << 32)

#define DIFROMREG(di,x)	((di) = \
			ctx->regs[x & ~((xcp->cp0_status & ST0_FR) == 0)])
#define DITOREG(di,x)	(ctx->regs[x & ~((xcp->cp0_status & ST0_FR) == 0)] \
			= (di))
#define DIFROMREG(di,x)	((di) = \
			ctx->regs[x & ~((xcp->cp0_status & ST0_FR) == 0)])
#define DITOREG(di,x)	(ctx->regs[x & ~((xcp->cp0_status & ST0_FR) == 0)] \
...
#define SIFROMREG(si,x)	((si).bits = ctx->regs[x])
#define SITOREG(si,x)	(ctx->regs[x] = (si).bits)

/* Beware: MIPS COP1 doubles are always little_word endian in registers */
#define DIFROMREG(di,x)	\
  ((di).bits = ((u64)ctx->regs[(x)+1] << 32) | ctx->regs[x])
#define DITOREG(di,x) \
  (ctx->regs[x] = (di).bits, ctx->regs[(x)+1] = (di).bits >> 32)
#endif
...
dcopuop:
		...
		default:
			if (MIPSInst_FUNC(ir) >= fcmp_op) {
				...
				DPFROMREG(fs, MIPSInst_FS(ir));
				DPFROMREG(ft, MIPSInst_FT(ir));
				rv.w = ieee754dp_cmp (fs, ft, cmptab[cmpop & 0x7], cmpop & 0x8);
				rfmt = -1;
				if ((cmpop & 0x8) && ieee754_cxtest (IEEE754_INVALID_OPERATION))
					rcsr = FPU_CSR_INV_X | FPU_CSR_INV_S;
				else
